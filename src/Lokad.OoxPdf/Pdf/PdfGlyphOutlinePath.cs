...
internal static class PdfGlyphOutlinePath
{
        ...
        double shear)
    {
        ...
        {
            AppendContourPath(contour);
        }

        void AppendContourPath(OpenTypeFont.OpenTypeGlyphContour contour)
        {
            ...
            DPoint[] points = BuildPdfContourPoints();
            ...
            DPoint[] BuildPdfContourPoints()
            {
                ...
                {
                    DPoint expandedCurrent = normalized[i];
                    DPoint next = normalized[(i + 1) % normalized.Count];
                    expanded.Add(expandedCurrent);
                    if (!expandedCurrent.IsOnCurve && !next.IsOnCurve)
                    {
                        expanded.Add(Midpoint(expandedCurrent, next));
                    }
                }
