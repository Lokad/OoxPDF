...
public static class OoxPdfConverter
{
    ...
    public static void Convert(string inputPath, string outputPath, OoxPdfOptions? options)
    {
        ...
        {
            OoxPdfInputKind.Pptx => new PptxRenderer(options.FontResolver).RenderPages(new PptxReader().Read(package), package, options.DiagnosticSink),
            OoxPdfInputKind.Docx => new DocxRenderer(options.FontResolver).RenderBlankPages(new DocxReader().Read(package, options.DiagnosticSink), options.DiagnosticSink),
