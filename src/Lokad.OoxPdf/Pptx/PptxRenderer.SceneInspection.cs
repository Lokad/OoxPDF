...
internal sealed partial class PptxRenderer
{
    ...
    private static PptxSceneNodeSnapshot ToSnapshot(PptxSceneNode node)
    {
        ...
        IReadOnlyList<int> chartStyleEntrySourceIndexes = chartStyleEntries
            ...
            .ToArray();
        IReadOnlyList<string> chartStyleEntryNamespaceUris = chartStyleEntries
            .Select(entry => entry.NamespaceUri)
            .ToArray();
        ...
        return new PptxSceneNodeSnapshot(
            ...
            node.Picture?.Line.HasLine ?? false,
            node.Picture?.Line.Width ?? 0d,
            node.Picture?.Line.Alpha ?? 1d,
            node.Picture?.Line.WidthSpecified ?? false,
