...
internal sealed partial class PptxRenderer
{
    ...
    private static IReadOnlyList<PdfFontResource> RenderChartTitle(PptxDocument document, PptxTheme theme, PdfGraphicsBuilder graphics, ShapeBounds bounds, XDocument chartXml, PptxSceneChart? sceneChart)
    {
        ...
        double height = OoxUnits.EmuToPoints(bounds.Height);
        double y = document.SlideHeightPoints - yTop - height;
        bool isAutoTitle = IsAutoGeneratedChartTitle(sceneChart, chartXml);
    ...
    private static bool IsAutoGeneratedChartTitle(PptxSceneChart? sceneChart, XDocument chartXml)
    {
        ...
        return string.IsNullOrWhiteSpace(sceneChart?.Title.Text) &&
            chartXml.Descendants(ChartNamespace + "chart").FirstOrDefault()?.Element(ChartNamespace + "title") is null &&
            !IsOoxmlBooleanElementEnabled(chartXml.Descendants(ChartNamespace + "autoTitleDeleted").FirstOrDefault());
    }
