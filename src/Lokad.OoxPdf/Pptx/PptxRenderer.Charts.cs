...
internal sealed partial class PptxRenderer
{
    ...
    private static bool TryRenderChart(PdfGraphicsBuilder graphics, PptxDocument document, PptxTheme theme, IReadOnlyList<RgbColor>? chartPalette, ShapeBounds bounds, XDocument chartXml, PptxSceneChart? sceneChart, List<PdfFontResource> fonts)
    {
        ...
        if (barChart is not null)
        {
            ...
            if (barSeries.Count != 0)
            {
                ...
                foreach (XElement comboLineChart in ReadChartPlotElements(chartXml, "lineChart"))
                {
                    ...
                    ChartValueExtents lineValueExtents = ReadSceneOrXmlChartValueAxisExtents(lineValueSceneAxis, lineValueAxisForScale, GetLineChartValueExtents(lineSeries, lineStacked, linePercentStacked), useNearMaximumHeadroom: true);
                    ...
                    legendEntries.AddRange(BuildStrokeLegendEntries(theme, chartPalette, linePlot, comboLineChart, lineSeriesStrokes, reverseOrder: lineStacked));
                    RenderLineChart(
                        graphics,
                        theme,
                        chartPalette,
        ...
        if (lineChart is not null)
        {
            ...
            if (lineSeries.Count != 0)
            {
                ...
                ChartValueExtents valueExtents = ReadSceneOrXmlChartValueAxisExtents(valueSceneAxis, valueAxisForScale, GetLineChartValueExtents(lineSeries, stacked, percentStacked), useNearMaximumHeadroom: true);
                ...
                fonts.AddRange(RenderChartLegend(graphics, chartLayout.Frame, plotBox, BuildStrokeLegendEntries(theme, chartPalette, linePlot, lineChart, seriesStrokes, reverseOrder: stacked), chartLayout.Legend, ReadSceneOrXmlChartLegendTextStyle(theme, sceneChart, chartXml)));
                    fonts.AddRange(RenderLineDataLabels(
                        theme,
                        graphics,
                        plotBox,
        ...
        if (pieChart is not null)
        {
            ...
            if (pieSeries.Count != 0)
            {
                ...
                ChartPlotBox plotBox = GetPolarChartPlotBox(document, bounds, chartXml, sceneChart);
                RenderChartAreaStyle(graphics, document, bounds, chartXml, sceneChart, theme);
                RenderPieChart(graphics, theme, chartPalette, plotBox, pieSeries[0], pointFills, pointStrokes, pointExplosions, ReadSceneOrXmlFirstSliceAngle(piePlot, pieChart));
        ...
        if (doughnutChart is not null)
        {
            ...
            if (doughnutSeries.Count != 0)
            {
                ...
                ChartPlotBox plotBox = GetPolarChartPlotBox(document, bounds, chartXml, sceneChart);
                RenderChartAreaStyle(graphics, document, bounds, chartXml, sceneChart, theme);
                RenderDoughnutChart(graphics, theme, chartPalette, plotBox, doughnutSeries[0], pointFills, pointStrokes, pointExplosions, holeSize, ReadSceneOrXmlFirstSliceAngle(doughnutPlot, doughnutChart));
    ...
    private static ChartPolarGeometry GetPieChartGeometry(ChartPlotBox plotBox, double explosionReserve = 0d)
    {
    ...
    private static IReadOnlyList<ChartLegendEntry> BuildCategoryFillLegendEntries(PptxTheme theme, IReadOnlyList<RgbColor>? chartPalette, PptxSceneChartPlot? plot, XElement chartElement, IReadOnlyDictionary<int, ChartSeriesFill> pointFills)
    {
        IReadOnlyList<string> labels = ReadSceneOrXmlCategoryLabels(plot, chartElement);
    ...
    private static IReadOnlyList<ChartLegendEntry> BuildStrokeLegendEntries(PptxTheme theme, IReadOnlyList<RgbColor>? chartPalette, PptxSceneChartPlot? plot, XElement chartElement, IReadOnlyList<ChartSeriesStroke?> seriesStrokes, bool reverseOrder = false)
    {
        IReadOnlyList<string> names = ReadSceneOrXmlChartSeriesNames(plot, chartElement);
        var entries = new List<ChartLegendEntry>(names.Count);
        for (int i = 0; i < names.Count; i++)
    ...
    private static IReadOnlyList<PdfFontResource> RenderPieDataLabels(PptxTheme theme, PdfGraphicsBuilder graphics, ChartPlotBox plotBox, IReadOnlyList<double> values, IReadOnlyDictionary<int, double> pointExplosions, double holeSize, ChartDataLabelOptions labelOptions)
    {
        ...
        ChartPolarGeometry geometry = GetPieChartGeometry(plotBox, explosionReserve);
        ...
        if (lineChart is not null)
        {
            ...
            if (series.Count > 0)
            {
                XElement? valueAxis = ReadChartValueAxisForChart(chartXml, lineChart) ??
                    chartXml.Descendants(ChartNamespace + "valAx").FirstOrDefault();
                PptxSceneChartAxis? valueSceneAxis = ReadSceneChartAxis(sceneChart, linePlot, "valAx");
                ChartValueExtents valueExtents = ReadSceneOrXmlChartValueAxisExtents(valueSceneAxis, valueAxis, GetLineChartValueExtents(series, stacked, percentStacked), useNearMaximumHeadroom: true);
                ChartAxisUnits axisUnits = ReadSceneOrXmlChartValueAxisUnits(valueSceneAxis, valueAxis);
                ...
                string[] tickLabels = tickValues
                    .Select(value => FormatSceneOrXmlChartAxisLabel(value, valueSceneAxis, valueAxis))
                    .ToArray();
    ...
    {
        RenderPieOrDoughnutSlices(graphics, theme, chartPalette, plotBox, values, pointFills, pointStrokes, pointExplosions, holeSize: 0d, firstSliceAngle);
    }
    ...
    {
        RenderPieOrDoughnutSlices(graphics, theme, chartPalette, plotBox, values, pointFills, pointStrokes, pointExplosions, holeSize, firstSliceAngle);
    }
    ...
    private readonly record struct ChartDataLabelOptions(bool ShowValue, bool ShowPercent, bool ShowCategoryName, bool ShowSeriesName, bool ShowLeaderLines, string CustomText, string Position, string Separator, string NumberFormat, PptxSceneChartManualLayout Layout, ChartTextStyleOverride TextStyle, ChartShapeStyle ShapeStyle, IReadOnlyDictionary<int, ChartDataLabelOverride> Overrides, bool IsDefined)
    {
        public static ChartDataLabelOptions None { get; } = new(ShowValue: false, ShowPercent: false, ShowCategoryName: false, ShowSeriesName: false, ShowLeaderLines: false, CustomText: string.Empty, Position: string.Empty, Separator: string.Empty, NumberFormat: string.Empty, Layout: default, TextStyle: ChartTextStyleOverride.Empty, ShapeStyle: ChartShapeStyle.Empty, Overrides: EmptyChartDataLabelOverrides, IsDefined: false);
    ...
    private readonly record struct ChartDataLabelOverride(bool? ShowValue, bool? ShowPercent, bool? ShowCategoryName, bool? ShowSeriesName, bool? ShowLeaderLines, string CustomText, string Position, string Separator, string NumberFormat, PptxSceneChartManualLayout Layout, ChartTextStyleOverride TextStyle, ChartShapeStyle ShapeStyle);
