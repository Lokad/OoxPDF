...
- [x] (2026-05-14) Add PPTX corporate-theme visual case.
...
- [x] (2026-05-14) Extend PNG parsing to 8-bit grayscale, 8-bit indexed color, and packed low-bit-depth indexed color.
- [ ] Implement Adam7 interlaced PNG decoding so embedded interlaced images render instead of being skipped.
- [ ] Implement non-skipping fallbacks for unsupported embedded images: preserve or rasterize every embedded image when possible, and treat omitted image content as a release-blocking diagnostic.
...
- [ ] Improve PPTX picture placement for grouped, placeholder-bound, and densely image-composed slides.
...
- [ ] Implement PPTX chart fallback rendering, starting with cached chart images or static chart drawing for common bar/line charts.
- [ ] Implement PPTX text run spacing fidelity: preserve spaces, kerning-like advance behavior, and mixed bold/italic runs without word fragmentation.
- [ ] Implement PPTX text frame fidelity: internal margins, vertical anchoring, clipping, overflow behavior, and shape text layout.
- [ ] Implement PPTX transparency and soft effects for fills, images, and overlay rectangles.
...
- [x] (2026-05-14) Run full build, tests, and pack from a clean checkout.
...
- [x] (2026-05-14) Complete first retrospective entry with remaining gaps and next implementation priorities.
...
- Observation: The corporate-theme PPTX case exercises inherited master shapes and exposes text-frame vertical placement as the next PPTX fidelity gap.
  Evidence: `pptx-corporate-theme` run `20260514-150712` has matching 1920 by 1080 reference/candidate images, mean absolute error `2.7617622733410494`, changed pixel ratio at threshold 16 of `0.02691502700617284`, empty diagnostics, and an assessment rating of 3. The assessment records that the master banner, rule, background, footer, local text, KPI rectangle, and ellipse render, but text inside local shapes sits too high or clips.

- Observation: A private large PPTX conversion exposed embedded image coverage as a release-critical gap.
  Evidence: The private conversion-only run `artifacts/private-visual/lokad-value-based/manual-20260514-153533` completed with diagnostics for 9 unsupported charts and 2 unsupported interlaced RGBA PNG image occurrences. The renderer now emits `IMAGE_UNSUPPORTED_FORMAT` instead of crashing, but the desired behavior is to render embedded images rather than skip them.

- Observation: The first private large PPTX visual assessment shows broad PPTX fidelity gaps beyond diagnostics-visible unsupported features.
  Evidence: Private run `artifacts/private-visual/lokad-value-based/20260514-154018` produced 84 comparison pages at 72 DPI. Mean absolute error is `17.34937080899104`, max mean absolute error is `76.6187287808642`, and mean changed-pixel ratio at threshold 16 is `0.1864059055335096`. Diagnostics show 9 unsupported chart occurrences and 2 unsupported interlaced PNG occurrences. Manual private inspection indicates missing or inaccurate capabilities in chart fallback, interlaced image rendering, dense image/group placement, text spacing, text frame anchoring/clipping, and transparent overlays.
...
- Outcome: Phase 0, blank-page conversion, visual comparison scaffolding, and first simple PPTX shape rendering are implemented. The repository builds with `Lokad.OoxPdf.slnx`, the library has the planned public API shell, the CLI can produce PDFs for recognized PPTX and DOCX inputs, and the visual harness creates Office reference PNGs, candidate PDFs, PDFium candidate PNGs, comparison metrics, HTML indexes, and assessment files. VisualDiff writes `metrics.json` and `index.html`, reads common grayscale, indexed, RGB, and RGBA PNGs, and computes dimensions plus simple pixel metrics.
  Validation: `dotnet build Lokad.OoxPdf.slnx --tl:off --nologo -v minimal` succeeds with 0 warnings and 0 errors. `dotnet run --project tests/Lokad.OoxPdf.Tests --tl:off` prints `46 passed, 0 failed`. `dotnet pack src/Lokad.OoxPdf/Lokad.OoxPdf.csproj --tl:off --nologo -v minimal --no-restore` succeeds. The same build, test, and pack commands succeed from a fresh local clone. `pwsh tools/CheckVisualCase.ps1 -Case visual-cases/cases/pptx-blank/case.json`, `pwsh tools/CheckVisualCase.ps1 -Case visual-cases/cases/docx-blank/case.json`, `pwsh tools/CheckVisualCase.ps1 -Case visual-cases/cases/pptx-shapes/case.json`, `pwsh tools/CheckVisualCase.ps1 -Case visual-cases/cases/pptx-text/case.json`, `pwsh tools/CheckVisualCase.ps1 -Case visual-cases/cases/pptx-images/case.json`, `pwsh tools/CheckVisualCase.ps1 -Case visual-cases/cases/pptx-table/case.json`, `pwsh tools/CheckVisualCase.ps1 -Case visual-cases/cases/pptx-corporate-theme/case.json`, `pwsh tools/CheckVisualCase.ps1 -Case visual-cases/cases/docx-basic-paragraphs/case.json`, `pwsh tools/CheckVisualCase.ps1 -Case visual-cases/cases/docx-numbering/case.json`, `pwsh tools/CheckVisualCase.ps1 -Case visual-cases/cases/docx-images/case.json`, `pwsh tools/CheckVisualCase.ps1 -Case visual-cases/cases/docx-tables/case.json`, and `pwsh tools/CheckVisualCase.ps1 -Case visual-cases/cases/docx-headers-footers/case.json` all complete successfully on this machine. The final smoke visual runs `pptx-blank` `20260514-150349` and `docx-blank` `20260514-150402` both have zero pixel error. `pptx-corporate-theme` `20260514-150712` has matching dimensions, empty diagnostics, and an assessment rating of 3.
  Remaining gaps: Rendering covers simple PPTX solid backgrounds, rectangles, lines, ellipses, basic rotation/flip transforms, simple Latin text runs with basic style approximations, common theme color/font references, common master/layout inheritance, JPEG/PNG pictures with basic cropping, grouped shape coordinate transforms, fixed-grid tables with simple fills, black borders, and text, warning diagnostics for common unsupported PPTX slide features, and DOCX paragraphs with style defaults, paragraph/character styles, spacing, alignment, basic run formatting, page breaking, simple bullet/decimal list labels, inline JPEG/PNG images, fixed-width tables with simple fills, borders, and text, default header/footer text, page number field approximation, warning diagnostics for common unsupported DOCX features, deterministic output for stable inputs, CLI exit-code behavior, and release-readiness docs. Important fidelity gaps remain around non-skipping embedded image rendering for all common PNG variants, chart rendering or chart fallback images, dense image/group placement, PPTX text spacing and text-frame layout, transparency/soft effects, complex text shaping, rich Office effects, charts/SmartArt, precise DOCX numbering indents, table merges/styles, and advanced Word section behavior.
  Next target: Improve fidelity on the lowest-rated and private cases, starting with Adam7 interlaced PNG rendering, chart fallback handling, PPTX text spacing/text-frame layout, dense image placement, and DOCX numbering hanging indents and level text cleanup.
