...
- [x] 2026-05-27: Align auto-enabled line marker envelopes with Office's public line-marker PDF structure:
  the shared `PptxChartMarkerMetricRules.AutoLineChartMarkerSize` now uses the Office-observed `7 pt` envelope
  for chart-level `c:marker val="1"` series that do not carry their own `c:marker` node. This keeps auto
  markers distinct from explicitly styled `9 pt` line markers and from generic `4 pt` chart markers.
  Validation: focused non-slow `pptx-charts` passed (`53 passed, 0 failed, 0 skipped`); line-marker port run
  `20260527-143525` passed and improved `LegendSwatchCandidate` max bounds delta from `1.69 pt` to
  `0.73 pt`, and `MarkerCandidate`/`StrokeMarkerCandidate` from `1.73 pt` to `0.77 pt`; sparse/blank probe
  ...
  default sizing.
- [x] 2026-05-27: Resolve the sparse no-title/right-legend explicit-scale Cartesian vertical plot-box model.
  The public sparse/blank probe showed the same Office vertical plot-box ratios for the no-title right-legend
  area and line charts with explicit value-axis `min`/`max`/`majorUnit`, while the public auto-scaled
  line-marker port still needs the older auto-scale vertical reserve. The no-title/right-legend Cartesian
  layout now selects the Office-observed sparse top/height ratios only when the value axis carries an explicit
  scale; auto-scaled line charts keep their existing auto-layout ratios. The sparse manifest now gates
  `GridlineAxisPlotBoxCandidate` beside `ChartSeriesLineCandidate` and gates `ValueAxisTickLabel` text
  placement, so this alignment is structural at the PDF level. Validation: focused non-slow `pptx-charts`
  passed (`56 passed, 0 failed, 0 skipped`); sparse/blank probe run `20260527-182459` passed with region-0
  gridline max bounds delta reduced from `3.23 pt` to `1.91 pt`, region-1 vertical gridline extents exact
  (`315.27..493.00`), filled-region max bounds delta reduced from `2.20 pt` to `1.91 pt`, and value-axis
...
- [x] Generalize Office-style dash text-operation boundaries:
  ...
  `DashPunctuation` category, preserving ASCII hyphen and em dash boundaries through layout/emission. Regular
  spaces immediately following a boundary dash are carried as hidden advance, matching Office's tendency to
  start the next visible text operation at the following word rather than emitting a standalone space
...
- [x] Add the first whitespace-control interaction probe:
  ...
  word spans. After the later baseline/font-emission work, the probe now gates full decoded text-operation
  parity, positions within `0.75 pt`, and the Office `0.03 Tc` branch on later lines; the remaining raster
  drift is concentrated in the accented final line and is tracked as font/accent glyph metric parity, not
  whitespace-control handling.
...
Revision note, 2026-05-25: Added the first radar-specific chart text oracle. `ClassifyPdfChartText.ps1` now uses
`RadarSpokeGroupCandidate` path center/radius evidence to classify polar chart text as category-axis or value-axis
tick labels before falling back to broad plot-box buckets. Both public radar manifests now compare
`CategoryAxisTickLabel` and `ValueAxisTickLabel` structures within `15 pt`, so radar labels have a PDF-text gate
instead of being hidden inside generic `DataLabelText`. Validation: the two-series radar text/graphics gate passed
at `artifacts/visual/pptx-ladder-11-chart-radar-2series-port/20260525-192920`; the filled radar text/graphics
gate passed at `artifacts/visual/pptx-ladder-11-chart-radar-filled-port/20260525-192931`; the full public
`pptx-charts` visual family passed 28/28 at `artifacts/visual/reports/pptx-charts.json`; and
`dotnet pack src\Lokad.OoxPdf\Lokad.OoxPdf.csproj --tl:off --nologo -v minimal --no-restore` succeeded.
...
SSIM `0.984040`. The manifests keep broader envelope tolerances for polar filled-region bounds while requiring
...
Revision note, 2026-05-25: Tightened the chart oracle configuration surface without pretending that rectangular
polar envelopes are true annulus geometry. `ComparePdfGraphicsOperations.ps1` now supports
`BoundsToleranceByKind` and `UseBoundsToleranceForUnlistedKinds`, mirroring the existing per-kind path-geometry
tolerance behavior. `CheckVisualCase.ps1` now enters the chart-structure gate when either a global
`maxChartGraphicsStructureBoundsDelta` or a per-kind `maxChartGraphicsStructureBoundsDeltaByKind` is present. The
left/top/bottom/right-overlay doughnut legend manifests now use per-kind bounds entries for
`PolarPlotBoxCandidate` and `PolarSliceCandidate` instead of one global polar-envelope tolerance, while keeping the
tight `PolarSliceCandidate` path-geometry gate at `0.6 pt`.

The evidence preserved here is that polar rectangular bounds are still filled-region envelopes: after nearest-bounds
matching, the current max bounds deltas are `21.68 pt` for the left-legend probe, `19.87 pt` for top and bottom
legend probes, and `17.21 pt` for the right-overlay probe. Those broad values apply to both the polar plot-box union
and individual slice candidate bounds, while the annulus center/radius path geometry remains below the tight gate.
Validation: the four affected visual cases passed with per-kind-only bounds gates at
`artifacts/visual/pptx-ladder-11-chart-doughnut-left-legend-probe/20260525-212329`,
`artifacts/visual/pptx-ladder-11-chart-doughnut-top-legend-probe/20260525-212341`,
`artifacts/visual/pptx-ladder-11-chart-doughnut-bottom-legend-probe/20260525-212341`, and
`artifacts/visual/pptx-ladder-11-chart-doughnut-right-overlay-legend-probe/20260525-212341`; the full public
`pptx-charts` family passed 33/33 at `artifacts/visual/reports/pptx-charts.json` from the `20260525-212416` run.
...
`PptxSceneChartTitle.IsAutoGenerated`, and `ResolveAutoChartTitleTextStyle` applies Office's bold title
default while preserving the existing 120% font-size scale only when no explicit title font size is present.
This is a structural correction: inferred auto titles are chart semantics, not a real `c:tx/rich` text body,
so PDF placement should use the renderer's measured fallback text path rather than the chart-rich-text width
estimator. The focused unit test now also verifies that an absent auto title requests a distinct embedded
font face for Office's bold title default, without relying on fragile subset font names.

The public polar chart validation exposed the adjacent long-term gap that should remain open: after the
auto-title fix, `pptx-ladder-11-chart-pie-5-categories-port` and
`pptx-ladder-11-chart-doughnut-port` pass their structural gates, but doughnut legend text still shows
non-gated text-position differences. That should not be papered over with a doughnut-specific coordinate
nudge. The next durable step is to classify legend entry text and marker boxes as first-class Office-PDF
structure, then replace legend anchoring/reserve calculations with scene-owned layout evidence and public
gates.

Validation: focused `PptxSyntheticChartAutoTitleUsesOfficeScaledSize` passed (`1 passed, 0 failed, 0 skipped`);
focused non-slow `pptx-charts` passed (`43 passed, 0 failed, 0 skipped`); full non-slow console runner passed
(`262 passed, 0 failed, 7 skipped`); public visual cases
`visual-cases/cases/pptx-ladder-11-chart-pie-5-categories-port/case.json` and
`visual-cases/cases/pptx-ladder-11-chart-doughnut-port/case.json` passed at run `20260527-041217`; full
public `pptx-charts` visual family passed (`38 passed, 0 failed`) with report generated at
...
Revision note, 2026-05-27: Added a public Office-authored data-label legend-key probe at
`visual-cases/cases/pptx-ladder-11-chart-data-label-legend-keys-probe/case.json`, generated by
`tools/NewChartProbeFixtures.ps1 -DataLabelsOnly`. The committed public fixture is deliberately a
clustered-column chart only: it preserves observable Office evidence for filled data-label keys without
mixing in formula-only line/scatter chart caches that the active renderer cannot yet hydrate structurally.

This slice also exposed two gaps that should stay visible rather than being trimmed away. First, an earlier
three-chart version of the probe produced `PPTX_CHART_MISSING_CACHED_DATA` for the line and scatter charts:
Office preserved the workbook-side values, but the chart XML visible to the renderer did not contain the
cached points it currently relies on. Long-term chart parity needs workbook-backed chart-data hydration
instead of treating formula-only chart series as unavailable. Second, the current PDF graphics classifier
cannot yet separate data-label legend keys from ordinary series fills, plot markers, and legend swatches.
The new case therefore uses tight raster gates only, and structural legend-key gates should wait for a
first-class `DataLabelLegendKeyCandidate` or equivalent classifier bucket backed by inspected Office PDFs.

Validation: the new case passed at run `20260527-195451` with mean absolute error
`2.297103226273148`, changed-pixel ratio at threshold 16 of `0.028956404320987655`, matching page count and
