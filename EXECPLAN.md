...
- [ ] 2026-06-01: Continue DOCX vertical-flow work from Office-observed page text and table structure, not
  ...
  2026-06-02 architecture follow-up: vertical-merge continuations now carry a typed `DocxVerticalMergeOwner`
  with the owner cell, owner row index, and owner grid column index; layout snapshots expose the two
  private-safe coordinates. This is the missing provenance for a future merged-cell fragment model where owner
  content can be clipped/emitted on continuation pages without rediscovering the restart cell from table
  structure at render time. Bottom-up coverage asserts same-page and cross-page owner coordinates, plus
...
- [ ] 2026-05-31: Pivot the active architecture track to DOCX after the slide-44 PPTX cleanup. Build a DOCX
  ...
  - [ ] 2026-05-31: Continue moving DOCX toward typed intermediate stages: split style-resolved paragraph/table
    ...
    2026-06-04 table-grid architecture follow-up: table layout now resolves preferred width, indent,
    explicit/missing grid, effective columns, scale, and resolved column widths through one
    `DocxResolvedTableGrid` plan shared by the keep-with-next first-row estimator and the actual
    `DocxTableLayoutFrame`. This removes duplicated width/grid logic between pagination estimation and row
    placement. Bottom-up coverage `DocxTableLayoutStageEstimatesKeptTableFirstRowWithResolvedPreferredWidth`
    verifies that a kept paragraph uses the same resolved preferred table width as real row layout, so wrapped
    first-row cell text contributes to pagination before the heading is placed. Validation passed
    `docx-tables --skip-slow` (`132`), `docx-core --skip-slow` (`63`), and full solution build.
...
- [ ] DOCX style-resolved block model: move from reader-flattened paragraph metrics toward a layout-ready
  ...
  2026-06-03 text-layout continuation: migrated the remaining DOCX layout stragglers for paragraph style
  provenance snapshots, paragraph indent offsets, list-label font-size measurement/segments, and
  table-cell column-break paragraph merge spacing to effective paragraph/run properties. A targeted scan of
  `DocxLayout.cs` no longer finds the direct scalar reads covered by this continuation. Validation passed
  full solution build, `docx-text --skip-slow` (`49`), `docx-page --skip-slow` (`45`), and `docx-tables
  --skip-slow` (`123`).
...
- [ ] DOCX Office PDF text-state decomposition:
  ...
  2026-06-03 reference-width-signature follow-up: `PdfInspect` text operations now include a private-safe
  `WidthSignature` derived from the reference PDF font width map and encoded payload: code count, pair count,
  total width units, and pair/left/right width ranges. `SummarizeDocxTextState.ps1` refreshes stale inspected
  PDF text-operation JSON that lacks this field and joins the reference PDF width ranges with candidate
  planner glyph ranges in paired summaries. Public rerun summaries for `docx-ladder-03-text-state-context`,
  `docx-ladder-03-text-state-size-matrix`, and `docx-ladder-03-text-state-font-matrix` now have reference
  width signatures on every nonzero `Tc` pair (`11`, `19`, `19`; no missing reference widths), with
  `ReferenceTcByReferenceFontSizeAndPdfWidthSideRange` and `ReferenceVsPlannerPdfWidthSideRange` buckets
  available for the next oracle pass. This still deliberately leaves rendering unchanged: early paired data
...
- DOCX section-break page-boundary validation:
  ...
  2026-06-02 architecture follow-up: explicit table-cell page-break row fragments now honor the active page frame
  before applying the authored split. If the measured pre-break fragment does not fit in the remaining frame and the
  frame already has content, `DocxLayout` starts a new page, repeats table headers through the normal path when needed,
  and only then emits the `CellPageBreak` fragments. This removes the under-margin placement weakness without adding a
  page-bottom slack scalar. Bottom-up coverage places a filler row before a cell-page-break row and verifies the first
  authored fragment starts on the next page inside the content frame. Validation passed `docx-tables --skip-slow`
...
- 2026-06-03 DOCX footnote reservation follow-up: footnote story bodies now reserve a page-owned area during
  ...
  2026-06-03 follow-up: inline-reference placement now carries the owning `DocxParagraph` through a layout-owned
  reference location and compares it with page text-line provenance. Table-cell text lines use their enclosing
  table source block plus exact paragraph identity, so repeated local run indexes in different cells/rows no
  longer let an earlier table paragraph claim a later footnote marker. Bottom-up coverage adds a split-page
