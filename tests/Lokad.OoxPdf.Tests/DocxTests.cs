...
internal static class DocxTests
{
    ...
    public static void DocxUnsupportedStoryDiagnosticsPreferRelatedPartNames()
    {
        ...
        DocxLayoutPageSnapshot relatedStoryPage = layoutSnapshot.Pages.Single();
        TestAssert.True(relatedStoryPage.PlacedRelatedStoryCount == 1 && relatedStoryPage.PlacedFootnoteStoryCount == 1 && relatedStoryPage.PlacedEndnoteStoryCount == 0, "Resolved footnote story bodies should be placed on the marker page while endnotes remain measured but unplaced until endnote-story flow is modeled.");
        TestAssert.True(relatedStoryPage.PlacedRelatedStoryTextLineCount >= 1 && relatedStoryPage.PlacedRelatedItems.Any(item => item.Kind == "PlacedTextLine" && item.SourceBlockIndex == 0), "Placed related-story snapshots should expose page-owned footnote text lines without flattening them into body items.");
        TestAssert.True(relatedStoryPage.PlacedRelatedItems.All(item => item.Y >= relatedStoryPage.MarginBottom), "Placed related-story items should be shifted from the unpaged story canvas into the page footnote area.");
