...
foreach (PptxSlide slide in slides)
{
    ...
    foreach (PptxTextGlyphRunSnapshot run in PptxRenderer.InspectTextGlyphRuns(document, package, slide.Index))
    {
        ...
        records.Add(new PptxGlyphRunRecord(
            ...
            run.FontFamily,
            run.ResolvedFontFamily,
            run.ResolvedFontIsFallback,
            run.ResolvedFontBold,
            run.ResolvedFontItalic,
            run.ResolvedFontWeightClass,
            run.ResolvedFontFaceIndex,
            run.ResolvedFontHasMathTable,
            run.ResolvedFontHasGposTable,
            run.ResolvedFontHasKernTable,
            run.ResolvedFontUnitsPerEm,
            run.ResolvedFontGlyphCount,
            run.ResolvedFontOs2Version,
            run.ResolvedFontOs2WidthClass,
            run.ResolvedFontOs2TypographicAscender,
            run.ResolvedFontOs2TypographicDescender,
            run.ResolvedFontOs2TypographicLineGap,
            run.ResolvedFontOs2WindowsAscender,
            run.ResolvedFontOs2WindowsDescender,
            Round(run.ResolvedFontPostItalicAngle),
            run.ResolvedFontPostUnderlinePosition,
            run.ResolvedFontPostUnderlineThickness,
            run.ResolvedFontPostIsFixedPitch,
            run.Bold,
            run.Italic,
            run.Underline,
            run.Strike,
...
static PptxTextFrameRecord ToFrameRecord(int slideNumber, int frameIndex, PptxTextFrameModelSnapshot frame)
{
    return new PptxTextFrameRecord(
        ...
        RoundNullable(frame.TableDeclaredRowHeight),
        RoundNullable(frame.TableDeclaredRowSpanHeight),
        RoundNullable(frame.TableDeclaredHeight),
        RoundNullable(frame.TableHeightSlackFactor),
